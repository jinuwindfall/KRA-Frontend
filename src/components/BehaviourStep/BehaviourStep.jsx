...
import styles from "./BehaviourStep.module.css";
import { getFinalMark, getSectionMetrics, getSectionWeight } from "../../utils/ratingUtils";

/**
 * BehaviourStep – identical table pattern to KRAObjectivesStep but for behaviour KRAs.
 */
export default function BehaviourStep({ appraisal = {}, kras = [], marks = {}, setMarks, role = "staff", canEditMarks = true, onBack, onNext }) {
  ...
  if (kras.length === 0) {
    return (
      <div className={styles.card}>
        <h2>Behaviour</h2>
        <p className={styles.subText}>No behaviour entries found.</p>
  ...
  return (
    <div className={styles.card}>
      <h2>Behaviour</h2>
      <p className={styles.subText}>Enter scores based on behaviour criteria.</p>
              ...
              <div className={styles.itemHeader}>
                <div>
                  <div className={styles.itemTitle}>
                    <span className={styles.serial}>#{kra.sl_no ?? index + 1}</span>
                    <strong>{kra.title || `Behaviour Row ${kra.sl_no ?? index + 1}`}</strong>
                  </div>
                  {kra.description ? (
                    <p className={styles.desc}>{kra.description}</p>
                  ) : (
                    <p className={styles.desc}>Appraiser will add the content for this row.</p>
                  )}
                </div>
                ...
                <div className={styles.scoreGrid}>
                  ...
                  {isReviewerView && (
                    <div className={`${styles.scoreBox} ${styles.inactiveBox}`}>
                      <label>Final</label>
                      <div className={styles.final}>{finalValue}</div>
                    </div>
                  )}
                </div>
              )}
